...
class array final {
 ...
 public:
  ...
  reference operator[](const size_type index) noexcept { return elems_[index]; }
  ...
  reference back() noexcept { return elems_[size_ - 1]; }

  const_reference back() const noexcept { return elems_[size_ - 1]; }

  pointer data() noexcept { return static_cast<pointer>(elems_); }
...
std::weak_ordering operator<=>(const ds::array<T, size>& lhs,
  ...
  for (std::size_t i = 0; i < lhs.size(); ++i) {
    if (lhs[i] < rhs[i]) return std::weak_ordering::less;

    if (lhs[i] > rhs[i]) return std::weak_ordering::greater;
  }
